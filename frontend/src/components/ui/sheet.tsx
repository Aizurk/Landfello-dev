...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPrimitive.Portal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-emerald-950 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-emerald-900/10">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn("text-lg font-semibold text-emerald-950", className)} {...props} />
));
